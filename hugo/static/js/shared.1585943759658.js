var superstore = (function (exports) {
	...
	const loggedIn = () => {
	    ...
	    let item = js_cookie.get(ID_LOGGED_IN_KEY);
	    if (!item) {
	        return false;
	    }
	    return true;
	...
	const logout = (redirect) => {
	    ...
	    js_cookie.remove(ID_LOGGED_IN_KEY);
	    localStorage.clear();
	    if (redirect === "#") {
	        return;
	    }
	...
	const login = (token, redirect) => {
	    js_cookie.set(ID_LOGGED_IN_KEY, token);
	    if (!redirect) {
	        redirect = '/welcome.html';
	    }
	    window.location = redirect;
...
//# sourceMappingURL=shared.1585943759658.js.map
