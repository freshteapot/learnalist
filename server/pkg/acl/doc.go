...
type AclWriterList interface {
	...
	GrantUserPublicListWriteAccess(userUUID string) error
	// Revoke a users right to write public lists
	RevokeUserPublicListWriteAccess(userUUID string) error
...
type AclReaderList interface {
	HasUserPublicListWriteAccess(userUUID string) (bool, error)
...
type EventPublicListAccessData struct {
	UserUUID string `json:"user_uuid"`
	Action   string `json:"action"`
}

const (
	EventPublicListAccess = "acl.publicListAccess"
)
