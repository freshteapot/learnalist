...
var _ = Describe("Testing AppleID Oauth redirect", func() {
	...
	BeforeEach(func() {
		...
		uriPrefix = "/api/v1/oauth/appleid/redirect"
		uri = uriPrefix
		e = echo.New()
	...
	It("Not enabled on server", func() {
		...
		service.V1OauthAppleIDRedirect(c)
		Expect(rec.Code).To(Equal(http.StatusInternalServerError))
		Expect(testutils.CleanEchoResponseFromResponseRecorder(rec)).To(Equal(`this website has not configured AppleID OAuth`))
	})
